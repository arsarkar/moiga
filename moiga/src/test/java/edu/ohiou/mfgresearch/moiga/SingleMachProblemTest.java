...
public class SingleMachProblemTest {
	...
	@Test
	public void testScheduleMutation(){
		List<Job> jobs = new LinkedList<Job>();
		jobs.add(new Job(1, 2.0, 1.0));
		jobs.add(new Job(2, 3.0, 3.0));
		jobs.add(new Job(3, 4.0, 4.0));
		...
		Variation variation = new SwapMutation(1.0);
		sols = variation.evolve(sols);
		for(Solution s:sols){
	...
	@Test
	public void ScheduleProblem_GASolver() throws Exception{
		List<Fuzzyficator> fuzzys = new LinkedList<Fuzzyficator>();
		fuzzys.add(new Fuzzyficator(new FuzzyMembershipT(20.0,0.0,20.0,0.0)));
		fuzzys.add(new Fuzzyficator(new FuzzyMembershipT(215.0,0.0,1.0,0.0)));
		...
		measures.add(PerformanceMeasures.MAXIMUM_TARDINESS);
		GASolver solver = 
				new GASolver(new ScheduleProblem(jobs,measures), 
							 fuzzys, 
							 (a,b)->a+b,
							 operators,
							 new Double[]{0.2, 0.5},
							 ...
							 new Random(),
							 sol->{},
							 pop_data->{System.out.println(pop_data.getBestCandidateFitness());});
		Solution ob=solver.evolve(100, 0, new GenerationCount(500)).copy();
		for(int i=0;i<ob.getNumberOfObjectives();i++)
			System.out.print(ob.getObjective(i)+" ");
		
	}
	
	@Test
	public void ScheduleProblem_GASolver3() throws Exception{
		List<Fuzzyficator> fuzzys = new LinkedList<Fuzzyficator>();
		fuzzys.add(new Fuzzyficator(new FuzzyMembershipT(20.0,0.0,2.0,0.1)));
		fuzzys.add(new Fuzzyficator(new FuzzyMembershipT(215.0,0.0,2.0,0.1)));
		//fuzzys.add(new Fuzzyficator(new FuzzyMembershipT(2367.0,0.0,900.0,800.0)));
		//fuzzys.add(new Fuzzyficator(new FuzzyMembershipT(215.0,0.0,140.0,60.0)));
		...
		Solution ob=solver.evolve(100, 0, populationData -> populationData.getBestCandidateFitness()>=2.0).copy();
