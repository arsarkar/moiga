...
public class FlowShopProblem implements Problem
{
	 ...
	 * @throws Exception 
	 * */
	public FlowShopProblem(List<Job> jobs,List<PerformanceMeasures> measures,int machineCount) throws Exception
	{
	...
	public void setCompletionTimes(List<Job> jobs)
	{
		double time[]=new double[machineCount];
		for(int i=0;i<jobs.size();i++)
		{
			if(time[jobs.get(i).machineID]<jobs.get(i).getReadyTime())
				time[jobs.get(i).machineID]=jobs.get(i).getReadyTime();
				time[jobs.get(i).machineID]+=jobs.get(i).getProcessingTime();
			jobs.get(i).setCompletionTime(time[jobs.get(i).machineID]);
		}
	}
